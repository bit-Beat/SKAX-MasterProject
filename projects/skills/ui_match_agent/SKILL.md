...
- `scenario_key`는 반드시 `ui_match`로 반환한다. `SC-003`, `sc_003`, `ui_match_agent`는 사용하지 않는다.
...
7. `persist_corrected_document_outputs("ui_match", "ui_match_agent", payload_json)`로 요구사항정의서, 기능정의서, UI설계서 보완본 3개를 생성한다.
